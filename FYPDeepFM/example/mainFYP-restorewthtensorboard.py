...
def _run_base_model_dfm(dfTrain, dfTest, folds, dfm_params,save_path:str,log_path:str,past_epoch:int=0):
    ...
    log_writer = tf.summary.FileWriter(log_path, graph=dfm.sess.graph)
    ...
    for i, (train_idx, valid_idx) in enumerate(folds):
        ...
        # continue
        output_merged_summary=dfm.fit(Xi_train_, Xv_train_, y_train_, Xi_valid_, Xv_valid_, y_valid_)
        for n,merged_summary in enumerate(output_merged_summary):
            # print(len(merged_summary))
            log_writer.add_summary(merged_summary)
...
def _plot_fig(train_results, valid_results, model_name):
    ...
    for i in range(train_results.shape[0]):
        plt.plot(xs, train_results[i], color="red", linestyle="solid", marker="o")
        plt.plot(xs, valid_results[i], color="blue", linestyle="dashed", marker="o")
    ...
    plt.legend(legends)
    plt.savefig("./fig/FYP%s.png"%model_name)
    plt.close()
...
# y_train_dfm, y_test_dfm,dfm_dfm = _run_base_model_dfm(dfTrain, dfTest, folds, dfm_params,"save/dfm/temp",past_epoch=20,log_path="logFYP/dfm")
...
y_train_dnn, y_test_dnn,dfm_dnn = _run_base_model_dfm(dfTrain, dfTest, folds, dnn_params,"save/dnn/temp",past_epoch=0,log_path="logFYP/dnn")
dfm_dnn.saver.save(dfm_dnn.sess,'save/dnn',global_step=dfm_params["epoch"])
