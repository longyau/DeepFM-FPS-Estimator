...
###load the CSV file according to the config python
def _load_data():
...
###initialize a DeepFM and restore the model based on the past epoch
###generate a csv recording all the estimated FPS data
### generate a graph to demonstrate the learning curve
...
###genereate a "submission.csv" to record the prediction result
def _make_submission(ids, y_pred, filename="submission.csv"):
    pd.DataFrame({"id": ids, "target": y_pred.flatten()}).to_csv(
        os.path.join(config.SUB_DIR, filename), index=False, float_format="%.5f")


###python function to generate a graph based on matplotlib
...
y_train_dnn, y_test_dnn,dfm_dnn = _run_base_model_dfm(dfTrain, dfTest, folds, dnn_params,"save/dnn/temp",past_epoch=56810)
